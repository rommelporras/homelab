...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: portfolio
        image: registry.k8s.home.rommelporras.com/0xwsh/portfolio:latest
        ports:
        - containerPort: 80
          name: http
        resources:
...
DNS is covered by the existing AdGuard wildcard: `*.k8s.home.rommelporras.com → 10.10.30.20`
...
kubectl-homelab set image deployment/portfolio \
  portfolio=registry.k8s.home.rommelporras.com/0xwsh/portfolio:<previous-tag> \
  -n portfolio-prod
