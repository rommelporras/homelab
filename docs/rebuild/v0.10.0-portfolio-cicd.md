...
**Architecture:**
```
feature/* → develop → [Build image] → deploy:development (auto)
                                           │
...
|----------|-------------|-----------|----------|
| KUBE_API_URL | All | Visible | No |
| KUBE_TOKEN | development | Masked and hidden | No |
| KUBE_TOKEN | staging | Masked and hidden | No |
| KUBE_TOKEN | production | Masked and hidden | No |

- `KUBE_API_URL`: `https://10.10.30.10:6443`
- `KUBE_TOKEN`: Token from each namespace's `gitlab-deploy-token`
- **1Password item:** `Portfolio` (vault: Kubernetes) — stores all CI/CD tokens

Set variables via CLI (from a separate trusted terminal):

```bash
# KUBE_API_URL (visible)
glab api -X POST projects/0xwsh%2Fportfolio/variables \
  --hostname gitlab.k8s.rommelporras.com \
  -f key=KUBE_API_URL \
  -f "value=$(op read 'op://Kubernetes/Portfolio/kube-api-url')" \
  -f masked=false -f hidden=false -f protected=false -f environment_scope='*'

# KUBE_TOKEN (masked and hidden, per environment)
for env in development staging production; do
  glab api -X POST projects/0xwsh%2Fportfolio/variables \
    --hostname gitlab.k8s.rommelporras.com \
    -f key=KUBE_TOKEN \
    -f "value=$(op read "op://Kubernetes/Portfolio/kube-token-${env}")" \
    -f masked_and_hidden=true -f protected=false -f environment_scope="${env}"
done
```
