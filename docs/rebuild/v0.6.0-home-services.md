...
metadata:
  ...
  namespace: home
  annotations:
    lbipam.cilium.io/ips: "10.10.30.53"
spec:
  type: LoadBalancer
  selector:
    app: adguard-home
...
dig @10.10.30.53 homepage.k8s.rommelporras.com
```

### 2.5 Update DHCP (OPNsense)

After verification, update OPNsense DHCP for all VLANs:
- Primary DNS: 10.10.30.53 (K8s AdGuard)
...
> **IMPORTANT:** Variable names must match `{{HOMEPAGE_VAR_XXX}}` placeholders in `services.yaml`.
> The `secret.yaml` in the repo is a template only — never apply it directly.
...
kubectl-homelab create secret generic homepage-secrets -n home \
  ...
  --from-literal=HOMEPAGE_VAR_OPNSENSE_KEY="$(op read 'op://Kubernetes/Homepage/opnsense-username')" \
  --from-literal=HOMEPAGE_VAR_OPNSENSE_SECRET="$(op read 'op://Kubernetes/Homepage/opnsense-password')" \
  ...
  --from-literal=HOMEPAGE_VAR_GLANCES_USER="glances" \
  ...
  --from-literal=HOMEPAGE_VAR_ADGUARD_FW_USER="$(op read 'op://Kubernetes/Homepage/adguard-fw-user')" \
  --from-literal=HOMEPAGE_VAR_ADGUARD_FW_PASS="$(op read 'op://Kubernetes/Homepage/adguard-fw-pass')" \
  --from-literal=HOMEPAGE_VAR_KARAKEEP_KEY="$(op read 'op://Kubernetes/Homepage/karakeep-key')" \
  ...
  --from-literal=HOMEPAGE_VAR_OPENWRT_USER="$(op read 'op://Kubernetes/Homepage/openwrt-user')" \
  --from-literal=HOMEPAGE_VAR_OPENWRT_PASS="$(op read 'op://Kubernetes/Homepage/openwrt-pass')" \
  --from-literal=HOMEPAGE_VAR_WEATHER_KEY="$(op read 'op://Kubernetes/Homepage/weather-key')"
```

### 5.3 Deploy via Kustomize

> **WARNING:** Always use `kubectl apply -k` (Kustomize), NOT `kubectl apply -f`.
> Using `-f` will apply `secret.yaml` (placeholder values) and overwrite real credentials,
> causing all widgets to fail with 401 errors.
...
- [ ] AdGuard DNS resolving: `dig @10.10.30.53 google.com`
- [ ] AdGuard web UI: https://adguard.k8s.rommelporras.com
- [ ] Homepage pods running (2 replicas): `kubectl-homelab get pods -n home -l app=homepage`
- [ ] Homepage accessible: https://homepage.k8s.rommelporras.com
...
- [ ] Longhorn UI accessible: https://longhorn.k8s.rommelporras.com
