...
2. **Discord webhook:** Configure integration to post results to `#speedtest` channel (webhook URL from `op://Kubernetes/Discord Webhooks/speedtest`, stored in MySpeed's built-in settings, not K8s manifests)
...
- [ ] Discord webhook posting to `#speedtest`
