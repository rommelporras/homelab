...
- **Discord #status** — Warnings, info, resolved alerts
...
| iCloud SMTP Alertmanager | Login | `username`, `password`, `server`, `port` |
...
op read "op://Kubernetes/iCloud SMTP Alertmanager/username" >/dev/null && echo "SMTP username OK"
op read "op://Kubernetes/iCloud SMTP Alertmanager/password" >/dev/null && echo "SMTP password OK"
```
...
username = subprocess.run(['op', 'read', 'op://Kubernetes/iCloud SMTP Alertmanager/username'],
                         capture_output=True, text=True).stdout.strip()
password = subprocess.run(['op', 'read', 'op://Kubernetes/iCloud SMTP Alertmanager/password'],
                         capture_output=True, text=True).stdout.strip()
...
# TestAlertWarning → discord-status
...
- [Phase 4.0: AdGuard Home](v0.6.0-adguard.md) (future)
