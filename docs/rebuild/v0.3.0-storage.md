...
for node in cp1 cp2 cp3; do
    echo "=== Preparing $node ==="
    ssh wawashi@$node.k8s.rommelporras.com "sudo mkdir -p /var/lib/longhorn && sudo chmod 700 /var/lib/longhorn"
    ssh wawashi@$node.k8s.rommelporras.com "df -h /var/lib/longhorn"
done
```
...
for node in cp1 cp2 cp3; do
    echo "=== $node ==="
    ssh wawashi@$node.k8s.rommelporras.com "systemctl is-active iscsid"
done
...
ssh cp1.k8s.rommelporras.com "df -h /var/lib/longhorn"
