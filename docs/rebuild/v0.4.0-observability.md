...
ssh wawashi@cp1.k8s.rommelporras.com 'sudo iptables-save | grep KUBE-SVC | wc -l'
# Should be 0
```
...
# Access Grafana
echo "URL: https://grafana.k8s.rommelporras.com"
echo "Login: admin"
...
ssh wawashi@cp1.k8s.rommelporras.com "sudo tee /etc/nut/upsd.users > /dev/null" <<EOF
[admin]
...
ssh wawashi@cp1.k8s.rommelporras.com "sudo chmod 640 /etc/nut/upsd.users && sudo chown root:nut /etc/nut/upsd.users"

# Configure upsmon for cp1 (master)
ssh wawashi@cp1.k8s.rommelporras.com "sudo tee /etc/nut/upsmon.conf > /dev/null" <<EOF
...
ssh wawashi@cp1.k8s.rommelporras.com "sudo chmod 640 /etc/nut/upsmon.conf && sudo chown root:nut /etc/nut/upsmon.conf"

# Start NUT services
ssh wawashi@cp1.k8s.rommelporras.com "sudo systemctl enable --now nut-server nut-monitor"

# Verify UPS detected
ssh wawashi@cp1.k8s.rommelporras.com "upsc cyberpower@localhost"
```
...
for node in cp2 cp3; do
    ssh wawashi@$node.k8s.rommelporras.com "sudo apt install nut-client -y"
    ssh wawashi@$node.k8s.rommelporras.com "echo 'MODE=netclient' | sudo tee /etc/nut/nut.conf"
done
```
...
```bash
ssh wawashi@cp3.k8s.rommelporras.com "sudo tee /etc/nut/upsmon.conf > /dev/null" <<EOF
...
ssh wawashi@cp3.k8s.rommelporras.com "sudo tee /etc/nut/upssched.conf > /dev/null" <<EOF
...
ssh wawashi@cp3.k8s.rommelporras.com "sudo tee /usr/local/bin/upssched-cmd > /dev/null" <<'SCRIPT'
#!/bin/bash
case $1 in
  early-shutdown)
...
ssh wawashi@cp3.k8s.rommelporras.com "sudo chmod +x /usr/local/bin/upssched-cmd && sudo chmod 640 /etc/nut/upsmon.conf && sudo chown root:nut /etc/nut/upsmon.conf"
```

Configure **k8s-cp2** (20-minute shutdown timer):

```bash
ssh wawashi@cp2.k8s.rommelporras.com "sudo tee /etc/nut/upsmon.conf > /dev/null" <<EOF
MONITOR cyberpower@10.10.30.11 1 monitor $(op read "op://Kubernetes/NUT Monitor/password") slave
MINSUPPLIES 1
POLLFREQ 5
POLLFREQALERT 2
...
ssh wawashi@cp2.k8s.rommelporras.com "sudo tee /etc/nut/upssched.conf > /dev/null" <<EOF
CMDSCRIPT /usr/local/bin/upssched-cmd
PIPEFN /run/nut/upssched.pipe
LOCKFN /run/nut/upssched.lock
...
ssh wawashi@cp2.k8s.rommelporras.com "sudo tee /usr/local/bin/upssched-cmd > /dev/null" <<'SCRIPT'
#!/bin/bash
case $1 in
  early-shutdown)
...
ssh wawashi@cp2.k8s.rommelporras.com "sudo chmod +x /usr/local/bin/upssched-cmd && sudo chmod 640 /etc/nut/upsmon.conf && sudo chown root:nut /etc/nut/upsmon.conf"
```
...
```bash
for node in cp2 cp3; do
    ssh wawashi@$node.k8s.rommelporras.com "sudo systemctl enable --now nut-monitor"
done
```
...
```bash
for node in cp1 cp2 cp3; do
    ssh wawashi@$node.k8s.rommelporras.com "sudo tee -a /var/lib/kubelet/config.yaml > /dev/null" <<EOF
shutdownGracePeriod: 120s
shutdownGracePeriodCriticalPods: 30s
EOF
    ssh wawashi@$node.k8s.rommelporras.com "sudo systemctl restart kubelet"
done

# Verify config
ssh wawashi@cp1.k8s.rommelporras.com "sudo grep -A1 shutdown /var/lib/kubelet/config.yaml"
```
...
for node in cp1 cp2 cp3; do
    echo "=== $node ==="
    ssh wawashi@$node.k8s.rommelporras.com "upsc cyberpower@10.10.30.11 battery.charge 2>/dev/null || upsc cyberpower@localhost battery.charge"
done
```
...
echo ""
echo "=== UPS Status ==="
ssh wawashi@cp1.k8s.rommelporras.com "upsc cyberpower@localhost ups.status"
ssh wawashi@cp1.k8s.rommelporras.com "upsc cyberpower@localhost battery.charge"

echo ""
echo "=== Access URLs ==="
echo "Grafana:       https://grafana.k8s.rommelporras.com"
echo "UPS Dashboard: https://grafana.k8s.rommelporras.com/d/ups-monitoring"
```
...
ssh wawashi@cp1.k8s.rommelporras.com "sudo systemctl status nut-server"

# Check USB connection
ssh wawashi@cp1.k8s.rommelporras.com "lsusb | grep -i cyber"

# Check NUT driver
ssh wawashi@cp1.k8s.rommelporras.com "sudo upsdrvctl start"
```
