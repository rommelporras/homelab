...
3. Store in 1Password (add `versions` field to existing `Discord Webhooks` item):

```bash
eval $(op signin)
# Discord Webhooks item already exists — add/update the versions field
op item edit "Discord Webhooks" --vault "Kubernetes" \
  "versions=<webhook-url>"
```
...
kubectl-homelab -n monitoring create secret generic discord-webhook \
  --from-literal=webhook-url="$(op read 'op://Kubernetes/Discord Webhooks/versions')"
```

5. Test webhook:

```bash
curl -X POST "$(op read 'op://Kubernetes/Discord Webhooks/versions')" \
