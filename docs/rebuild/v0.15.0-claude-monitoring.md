...
> **Prerequisite:** [v0.4.0 rebuild](v0.4.0-observability.md) completed (kube-prometheus-stack + Loki running)
...
# Health check responding (pod 1/1 Running = readiness probe on :13133 passing)
kubectl-homelab get pods -n monitoring -l app=otel-collector -o wide
...
Verify by checking the Helm-generated ConfigMap:
```bash
kubectl-homelab get cm loki -n monitoring -o jsonpath='{.data.config\.yaml}' | grep -A5 limits_config
```

Loki v3.6.3 defaults `allow_structured_metadata: true` even if not explicitly in the config.
...
- Overview / Productivity: cost, session stats, commits, PRs, lines of code
- Trends / Performance (Events): cost/token rate timeseries, API latency
- Sessions & Activity / Cost Analysis: active users, session length, spend breakdowns
- Token & Efficiency / Insights: cache hit rate, tool usage, CLI vs User time, event logs
