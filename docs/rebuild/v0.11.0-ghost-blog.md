...
### 6.2 WAF Rules — Protect `/ghost` Admin + Allow RSS

Create three WAF Custom Rules in Cloudflare Dashboard (Security → WAF → Custom Rules) **in this exact order**:

**Rule 1: Blog - Allow RSS Feed** (must be first)

```
Expression: (http.host eq "blog.rommelporras.com" and
             http.request.uri.path eq "/rss/")
Action: Skip → Skip all remaining custom rules + All Super Bot Fight Mode Rules
```

Why: Cloudflare Bot Management blocks cloud provider IPs (GitHub Actions, AWS, etc.). The RSS feed is public read-only content consumed by the GitHub Profile README blog-post workflow. Must also skip Super Bot Fight Mode — WAF custom rule skips alone are insufficient.

Also disable **Bot Fight Mode** globally (Security → Settings → Bot Fight Mode → OFF). The free Cloudflare tier cannot create path-specific Bot Fight Mode exceptions.

**Rule 2: Ghost - Allow Content API** (must be ABOVE Rule 3)
...
**Rule 3: Ghost - Block Admin** (must be last)
...
Why: Blocks `/ghost/` (admin panel), `/ghost/api/admin/` (Admin API), and all `/ghost/*` paths. Content API traffic never reaches this rule because Rule 2 skips it.

Verify:

```bash
# RSS feed works from any IP (200)
curl -sI https://blog.rommelporras.com/rss/ | head -5
