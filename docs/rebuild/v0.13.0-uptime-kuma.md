# v0.13.0 — Uptime Kuma Monitoring
...
**Version:** Uptime Kuma v2.0.2 (rootless — includes Chromium for browser-engine monitors)
...
This applies:
- **statefulset.yaml** — 1 replica, rootless image, Longhorn PVC via volumeClaimTemplates
- **service.yaml** — Headless service (StatefulSet identity) + ClusterIP (HTTPRoute backend)
- **httproute.yaml** — Gateway API route for `uptime.k8s.rommelporras.com`
- **networkpolicy.yaml** — CiliumNetworkPolicy for egress control
...
```
uptime.k8s.rommelporras.com → 10.10.30.20
```
...
| Discord Webhook URL | from 1Password (`op read "op://Kubernetes/Discord Webhooks/incidents"`) |
...
Click **Test** — verify "Discord Incidents Testing" appears in #incidents channel, then **Save**.
...
Add → Tag for each tag (used to categorize monitors across groups):
...
Add Monitor → Monitor Type: **Group** for each group:
1. **Website**
2. **Apps**
3. **Infrastructure**
4. **DNS**
...
Add Monitor → Monitor Type: **HTTP(s)** for each monitor. Set the **Parent Monitor** field to assign it to a group.

**Default settings for all monitors:**
- Heartbeat Interval: 60 seconds
- Heartbeat Retry Interval: 60 seconds
- Request Timeout: 48 seconds
- Resend Notification: 0 (disabled — you get DOWN and UP notifications, no repeats)
- Max Redirects: 10
...
- Kubernetes services (Grafana, Homepage Dashboard, Longhorn, AdGuard Primary, Blog, etc.) → **Kubernetes**
- Proxmox-hosted services (Proxmox PVE, Proxmox Firewall, Homepage Proxmox) → **Proxmox**
- Network devices (OPNsense, AdGuard Primary, AdGuard Failover) → **Network**
- Storage services (Longhorn, OpenMediaVault, NAS Glances) → **Storage**
- Publicly accessible services (rommelporras.com, Blog Prod, Staging) → **Public**
...
- Footer: `Monitored from a 3-node Kubernetes cluster 🏠`
...
1. **Website** — Add monitors: rommelporras.com, Blog Prod, rommelporras.com (Staging), Blog Dev
2. **DNS** — Add monitors: AdGuard Primary, AdGuard Failover
3. **Apps** — Add monitors: Grafana, Longhorn Storage, Homepage Dashboard, Immich, Karakeep, MySpeed, Homepage (Proxmox)
4. **Infrastructure** — Add monitors: OPNsense, Proxmox PVE, Proxmox Firewall, OpenMediaVault, NAS Glances
...
2. Click the **homelab** tunnel → **Public Hostname** tab
...
### 8c: Cloudflare Access — Block Admin Routes

This blocks admin UI paths while keeping the status page publicly accessible. We use a **block-admin** approach (block specific paths) instead of an allowlist (allow specific paths) because Uptime Kuma is an SPA that loads JS/CSS/API from various paths — an allowlist approach causes CORS errors when the SPA tries to load `/assets/`, `/api/status-page/heartbeat/`, `/socket.io/`, etc.

**First, create the reusable policy:**

1. Cloudflare Zero Trust → Access → Policies → **Add a policy**
...
3. Application domain — click **Add public hostname** for each:
...
- `status.rommelporras.com/status/homelab` → loads status page
- `status.rommelporras.com/dashboard` → Cloudflare Access block page
...
| volumeClaimTemplates | Auto-creates PVC per pod — no separate PVC manifest needed |
| Headless Service | `clusterIP: None` gives pods stable DNS names |
| CiliumNetworkPolicy ports | Uses pod ports, not service ports (e.g., Grafana service:80 → pod:3000) |
| Hairpin routing | Cilium Gateway returns 403 for pod-to-VIP-to-pod traffic; resolved after network policy updates |
| Cloudflare Access allowlist | Bypass `/status/homelab` + Block everything causes CORS — SPA loads JS/CSS/API from many paths |
| Cloudflare Access block-admin | Block only `/dashboard`, `/manage-status-page`, `/settings` — simpler, SPA-compatible |
...
| Accepted status codes | NAS Glances returns 401 (auth required) — add to accepted codes to confirm service is UP |
