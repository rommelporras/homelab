# v0.22.0 — Tailscale Operator (Subnet Router)
...
Includes Cilium socketLB compatibility fix, operator-only CiliumNetworkPolicies, PrometheusRule alerts, Grafana dashboard (interface-split traffic monitoring), and Homepage widget.
...
## Step 7: Apply Monitoring Alerts and Dashboard

```bash
kubectl-homelab apply -f manifests/monitoring/tailscale-alerts.yaml
kubectl-homelab apply -f manifests/monitoring/tailscale-dashboard-configmap.yaml

# Verify
kubectl-homelab -n monitoring get prometheusrule tailscale-alerts
kubectl-homelab -n monitoring get configmap tailscale-dashboard
```

Grafana dashboard "Tailscale Operator" auto-provisions via sidecar (grafana_dashboard label).
Panels: Pod Status, VPN Tunnel Traffic (tailscale0), Pod Network Traffic (eth0), Resource Usage.
