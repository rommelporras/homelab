...
56. **Task #25 - staging-promote sensor was broken by design; `data` filter with `{{env.X}}` never worked.** Original `sensor-portfolio-staging-promote.yaml` had `filters.data: [{path: headers.X-Staging-Promote-Token, type: string, value: ["{{env.STAGING_PROMOTE_TOKEN}}"]}]`. Investigation during Task #25 smoke uncovered three separate flaws in argo-events v1.9.10's `data` filter that individually or together silently rejected every legitimate POST: (a) the webhook EventSource publishes headers under `header.*` (singular), not `headers.*` (plural) - path didn't exist, filter errored (WARN `path 'headers.X-Staging-Promote-Token' does not exist`); (b) even after fixing the path to singular, argo-events applies string-type filter values as **regex** against `gjson pathResult.String()`, and for header arrays (`["value"]`) the String() rendering is the JSON literal (`["value"]`), not the bare scalar - so the regex never matched the token; (c) `{{env.STAGING_PROMOTE_TOKEN}}` is **not substituted** anywhere in `pkg/sensors/dependencies/filter.go` - the template string is taken verbatim and regex-compiled as `{{env.STAGING_PROMOTE_TOKEN}}` which matches only strings containing literal `{{env` + any char + `STAGING_PROMOTE_TOKEN}}`, never a real token. Switching to the `script` filter (gopher-lua, `os.getenv` available via default stdlib) fixed (b) and (c), but reading the header array from the mapToTable-produced Lua table also failed in this environment. Settled on body-based auth: payload is `{"token": "<secret>", "source_sha": "abc12345"}`, sensor script is 3 lines (`local body = event.body; if body == nil then return false end; return body.token == os.getenv("STAGING_PROMOTE_TOKEN")`). Workflow `portfolio-staging-promote-dxzmd` ran 2/2 green (deploy 16s → verify 6s, total 26s). CI-bot commit `ae7a6c0 chore(ci): update portfolio/staging image to db5f587c [skip ci]` landed on origin/main; ArgoCD rolled `portfolio-staging` Deployment to `:db5f587c` (gen 24, 2/2 pods Ready). **Secondary finding: sensor warn logs for filter-rejected events include the FULL event body (headers + body) in plaintext. Any auth token in a header or body leaks to pod logs on every rejection.** The `staging-promote-token` leaked repeatedly into sensor warn logs during this debug cycle and is scheduled for rotation in Task #27. Fix commit `b5059cd`. **Rule for future webhook auth filters:** avoid `data` filter with `type: string` + `{{env.X}}` template - it does not work. Use `script` filter with `os.getenv()`, put auth in request BODY (flat scalar), not headers (array), because mapToTable's array handling in the Lua-exposed event has unreliable semantics.
...
- [x] **Portfolio staging promotion flow** — Task #25, `portfolio-staging-promote-dxzmd` green 2/2 (2026-04-24, deploy 16s → verify 6s). CI-bot commit `ae7a6c0` landed, ArgoCD rolled `portfolio-staging` to `:db5f587c`. Sensor filter was totally broken (3 separate argo-events v1.9.10 `data` filter quirks) and had to be rewritten as a script filter on `body.token`. See Notes #56.
