...
|---|---|---|---|---|
| ~~`argo-workflows/gitlab-registry` password~~ | `secret/argo-workflows/gitlab-registry` | `op://Kubernetes/Argo Workflows/registry-password` | `glpat-sJlBSQ_…` appears in transcript (line ~555) | ⏳ **Rotated 2026-04-24 (write-path soak-verification pending)** — Vault bumped to v5; new GitLab group deploy token `argo-workflows-buildkit-2026-04-24` created alongside old `argo-workflows-buildkit`; ExternalSecret force-synced. Pull auth proven from in-cluster skopeo probe (listed all tags on `0xwsh/portfolio`). GitLab deploy tokens are binary `read_registry` + `write_registry` scope, so write will work when next CI build runs. Action after soak-confirm: delete old `argo-workflows-buildkit` deploy token in GitLab UI. |
...
| `invoicetron-prod/app` `database-url` + `invoicetron-prod/db` `postgres-password` | `secret/invoicetron-prod/*` | `op://Kubernetes/Invoicetron Prod/*` | same pattern as dev (JSON-dump / vault kv get) | ⏭ **PROD rotation pending** — plan same as dev, coordinate during low-traffic window; ~30-60s app downtime during ALTER USER + pod restart |
